...
ICR2 Animator is a Windows Python tool for animating objects in *IndyCar Racing II* / *CART Racing* running under DOSBox. It finds objects in DOSBox memory by their current in-game coordinates, then updates their position and rotation values every animation frame so trackside objects can move, shuttle, loop, or spin while the game is running.
